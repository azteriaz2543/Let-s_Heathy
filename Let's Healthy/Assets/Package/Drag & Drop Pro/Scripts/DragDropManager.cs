...
public class DragDropManager : MonoBehaviour {
    ...
    public static void ResetOnlyObject(string ObjectId)
    {
        for (int i = 0; i < DDM.AllObjects.Length; i++)
        {
            if (DDM.AllObjects[i].name == ObjectId)
            {
        ...
        {
            if (DDM.AllPanels[i].currentItem == ObjectId)
            {
                DDM.AllPanels[i].currentItem = "Reset";
            }
        }
